...
window.title("ThreatGuardian Antivirus")
...
def mainFrameNj():
    ...
    def customScanFunc():
        ...
        ri(source_path)
        # if virus found show dialog box 
        # else show no virus found dialog box
        messagebox.showinfo("Virus Scanner", "Virus Scan Completed")
        if len(engine.virusScanner(source_path)) > 0:
            messagebox.showinfo("Virus Found", str((engine.virusScanner(source_path))))
        else:
            messagebox.showinfo("Virus Scanner", "No Virus Found")
    ...
    ThreatGuardianMainLabel = Label(nj, text = "ThreatGuardian", font = "Times 30 bold")
    ThreatGuardianMainLabel.pack(side = TOP, pady = 20)
    ...
    quickScan.place( x = 90 , y = 350)

    # Text
    quickScanText = Label(nj, text = "Quick Scan", font = "Times 20 bold")
    quickScanText.place(x = 90, y = 500)
    
    fscan = Button(nj, image = photoScan, bd = 0, command = customScanFunc)
    fscan.place(x = 350 , y = 350)

    # Text
    fscanText = Label(nj, text = "Folder Scan", font = "Times 20 bold")
    fscanText.place(x = 350, y = 500)

    junkFileRemover = Button(nj, image = junkFileRemoverImg ,bd = 0, command = junkFileRemoverFunc)
    junkFileRemover.place( x = 610 , y = 350)

    # Text
    junkFileRemoverText = Label(nj, text = "Junk File Remover", font = "Times 20 bold")
    junkFileRemoverText.place(x = 590, y = 500)

    ramBooster = Button(nj, image = ramBoosterImg , bd = 0, command = ramBoosterFunc)
    ramBooster.place( x = 870 , y = 350)

    # Text
    ramBoosterText = Label(nj, text = "Ram Booster", font = "Times 20 bold")
    ramBoosterText.place(x = 870, y = 500)
...
def quickScanFrame():
    ...
    ThreatGuardianMainLabel = Label(nj, text = "Quick Scan", font = "Times 21 bold")
    ThreatGuardianMainLabel.pack(side = TOP, pady = 20)
